class User < ApplicationRecord
  ...
  with_options presence: true do
    ...
    validates :password, format: { with: PASSWORD_REGEX, message: "6文字以上で英字と数字を混同して設定する必要があります" }
    # ニックネーム必須
    validates :nickname
    
    # 本名：苗字と名前、全角（漢字・平仮名・カタカナ）
    with_options format: { with: /\A[ぁ-んァ-ン一-龥]/, message: '全角のみ' } do
      validates :family_name
      validates :first_name
    end
    # 振り仮名：苗字と名前、全角（カタカナ）/\A[ァ-ヶー－]+\z/
    with_options format: { with: /\A[ァ-ヶー－]+\z/, message: '全角のみ' } do
